...
public class LevelGenerator : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
                ...
                CreateChildPrefab(floorPrefab, floorParent, x, 0, z, currentTag);
        ...
        while (voids < 4)
        {
            GameObject currentObj = floorParent.transform.GetChild(Random.Range(32, mazeSize * mazeSize - 31)).gameObject;
            if (currentObj.tag == "noWall")
            {
	...
	void CreateChildPrefab(GameObject prefab, GameObject parent, int x, int y, int z, string tag = "") {
