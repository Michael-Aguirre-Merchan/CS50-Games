...
function Room:update(dt)
    ...
    for k, object in pairs(self.objects) do
        ...
        if object.thrown and not object.done then
            for i, ent in pairs(self.entities) do
                if ent:collides(object) then
                    object.done = true
                    ent:damage(1)
                end
            end
...
function Room:canPlayerLift()
    local currentX, currentY = self.player.x, self.player.y + self.player.height / 2 
    if self.player.direction == 'left' then
        currentX = currentX  - 1
    elseif self.player.direction == 'right' then
        currentX = currentX + self.player.width + 1
    elseif self.player.direction == 'up' then
        currentY = currentY - 1
    else
        currentY = currentY + self.player.height + 1
    end
    
    for i, x in pairs(self.objects) do
        if x:isPotThere(currentX, currentY, self.player.width, self.player.height / 2) then
            return x
        end
    end
end

function Room:spawnheart(px, py)
    local heart = GameObject(GAME_OBJECT_DEFS['heart'], px, py)
